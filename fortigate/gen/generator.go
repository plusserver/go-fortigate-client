...
func main() {
	...
	generate := map[string]map[string]string{
		"VIP":             { "path": "/cmdb/firewall/vip" },
		"FirewallAddress": { "path": "/cmdb/firewall/address" },
		"FirewallPolicy":  { "path": "/cmdb/firewall/policy" },
	}
...
type {{.TypeName}}Results struct {
  ...
  Mkey  {{ gotypestr .Results.MkeyType }} {{ bq }}json:"mkey"{{ bq }}
  Result
}
...
func (c *WebClient) Create{{.TypeName}}(obj *{{.TypeName}}) (id {{ gotypestr .Results.MkeyType }}, err error) {
  var errmsg Result
  var results {{.TypeName}}Results
  _, err = c.napping.Post(c.URL+"/api/v2{{ .Path }}", obj, &results, &errmsg)
	if err != nil {
    return {{ emptyliteralfor .Results.MkeyType }}, fmt.Errorf("error creating {{.TypeName}} '%s': %s", obj.Name, err.Error())
  }
  if results.HTTPStatus == 200 {
    return
  }
	if errmsg.HTTPStatus != 200 {
    return {{ emptyliteralfor .Results.MkeyType }}, fmt.Errorf("error creating {{.TypeName}} '%s': %s", obj.Name, errmsg.Status)
  }
...
// List all {{.TypeName}}s
func (c *FakeClient) List{{.TypeName}}s() (res []*{{.TypeName}}, err error) {
  for _, r := range c.{{.TypeName}}s {
    res = append(res, r)
  }
  return
}
...
// Get a {{.TypeName}} by name
func (c *FakeClient) Get{{.TypeName}}(name string) (*{{.TypeName}}, error) {
	if res, ok := c.{{.TypeName}}s[name]; ok {
		return res, nil
	} else {
		return &{{.TypeName}}{}, fmt.Errorf("error getting {{.TypeName}} '%s': not found", name)
	} 
...
// Create a new {{.TypeName}}
func (c *FakeClient) Create{{.TypeName}}(obj *{{.TypeName}}) (id {{ gotypestr .Results.MkeyType }}, err error) {
	c.{{.TypeName}}s[obj.Name] = obj
	return {{ emptyliteralfor .Results.MkeyType }}, nil
}

// Update a {{.TypeName}}
func (c *FakeClient) Update{{.TypeName}}(obj *{{.TypeName}}) (err error) {
	c.{{.TypeName}}s[obj.Name] = obj
	return nil
}

// Delete a {{.TypeName}} by name
func (c *FakeClient) Delete{{.TypeName}}(name string) (err error) {
	delete(c.{{.TypeName}}s, name)
	return nil
}
`
	...
	funcMap := template.FuncMap{
		...
		"gotypestr": func(s string) string {
			switch s {
			case "integer":
				return "int"
			default:
		...
		"getmkeytype": func(m map[string]string) string {
			switch m["mkey_type"] {
			case "integer":
				return "int"
			default:
	...
	for typ, data := range generate {
		schema := c.Schema(data["path"])

		schema.TypeName = typ
		schema.Path = data["path"]
		...
		data["mkey_type"] = schema.Results.MkeyType
	}
...
{{ range $a, $b := . }}
  ...
  // Create a new {{ $a }}
  Create{{ $a }}(*{{ $a }}) ({{ getmkeytype $b }}, error)
...
`
	...
	t, err = template.New("client").Funcs(funcMap).Parse(tt)
	if err != nil {
		panic(err)
	}
...
`
	...
	t, err = template.New("fake").Parse(tt)
	if err != nil {
		panic(err)
	}

	err = t.Execute(&buffer2, generate)
	if err != nil {
		panic(err)
	}

	err = ioutil.WriteFile("./fortigate/fake.go", buffer2.Bytes(), 0644)
	if err != nil {
		panic(err)
	}
